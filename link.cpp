...
{
	int cnt=k;
	node *tt=*head,*t1=*head,*prev=NULL,*temp,*current=*head;
	while(cnt--)
	{
		t1=t1->next;
	}
...
node *add(node **head1, node **head2)
{
	...
	while(t1!=NULL ||  t2!=NULL)
	{
		temp=(t1?t1->val:0)+(t2?t2->val:0)+carry;
		append(&t,createnode(temp%10));
		carry=(temp>=10)?1:0;
		if(t1) t1=t1->next;
		if(t2) t2=t2->next;
	}	
	if(carry)
	   append(&t,createnode(carry));

return t;
}
void rotatek(node **head, int k)
{
	node *t=*head,*prev=NULL,*lst=*head;
	while(lst->next)
	{
		lst=lst->next;
	}
